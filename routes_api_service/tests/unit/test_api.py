...
def test_correct(app):
    ...
    with app.test_client() as client:
        response = client.get(RATES_URL, query_string=dict(date_from="2016-01-01", date_to="2016-01-10", origin="CNSGH", destination="north_europe_main"))
        assert 200 == response.status_code
        assert expected == response.data


@pytest.mark.parametrize("query_string", [dict(date_from="2016-01-01", origin="A", destination="B"),
...
@pytest.mark.parametrize("query_string", [
    dict(date_from="207", date_to="2016-01-10", origin="A", destination="B"),
    dict(date_from="2016-01-01", date_to="2016A-01-10", origin="A", destination="B"),
                                          ])
